...
public class WebSocketService extends AbstractBaseServerService {
	...
	public static final String websocketReq = "/req/ws";

	static {
		pathMap.put(websocketReq, WebSocketConfiguration.class);
	}
		...
		Thread thread2 = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (String path : pathMap.keySet()) {
					...
					websocket.addServer(pathMap.get(path), path);
				}
				try {
					SeasonLogTools.getLog().info("thread2 启动对于:" + port + "监听");
					JettyServer.startJetty(websocket);
				} catch (Exception e) {
					e.printStackTrace();
					SeasonLogTools.getLog().error("  WebSocketService error ",
							e);
				}
			}
		});
